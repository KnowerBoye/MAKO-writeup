...
\noindent While DAG-based orchestration is effective for workflows with clearly defined sequences, it introduces limitations in large-scale systems that support multiple workflows involving varying combinations of agents and tools. In such environments, a separate DAG must typically be manually constructed and maintained for each workflow. 
...
\section{Objectives}
\begin{itemize}
    \item Design a knowledge graph–based multi-agent orchestration framework that defines the structure for task entry, agent representation, task delegation, and task completion.
    \item Develop an orchestrator agent responsible for coordinating agents and managing workflow execution within the framework.
    \item Design a knowledge graph traversal strategy to infer task sequencing and identify appropriate agents or tools for task execution.
    \item Evaluate the effectiveness of the proposed framework in dynamically generating workflows and coordinating multi-agent interactions.
    \item Implement a prototype application built on top of the framework to demonstrate its practical use and capabilities.
\end{itemize}


\section{Methods Used}
This research adopts a Design Science Research (DSR) methodology to guide the development and evaluation of the proposed system. Design Science Research focuses on the creation and assessment of technological artifacts intended to address identified problems. In this study, the artifact is a knowledge graph–based multi-agent orchestration framework designed to dynamically infer workflows in LLM-based multi-agent systems.
\begin{itemize}
